...
function CatalogManager({ catalog, setCatalog, saveCatalog, saveStatus }) {
  ...
  return <form onSubmit={saveCatalog} className="max-w-4xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-sand">Storefront controls</p><h1 className="mt-1 text-3xl font-semibold">Your offers, products & prices</h1><p className="mt-2 text-sm text-gray-600">Upload the four home offers and manage every visible product price here. Press Save when you finish.</p>
    ...
    <div className="mt-5 rounded-2xl bg-white p-5 shadow-sm"><div><h2 className="font-semibold">Home offer slideshow</h2><p className="mt-1 text-xs text-gray-500">Exactly four offer images. The website changes between them automatically every 2 seconds.</p></div><div className="mt-5 space-y-5">{slides.slice(0, 4).map((slide, i) => <div key={i} className="grid gap-3 rounded-xl border border-gray-100 p-3 sm:grid-cols-[120px_1fr]"><img src={slide.image} alt="" className="h-28 w-full rounded-lg object-cover sm:w-[7.5rem]" /><div className="grid gap-2"><input value={slide.title} onChange={e => setItem('slides', i, 'title', e.target.value)} className="field" placeholder="Offer title" /><input value={slide.subtitle} onChange={e => setItem('slides', i, 'subtitle', e.target.value)} className="field" placeholder="Short offer description" /><div className="flex gap-2"><input value={slide.image} onChange={e => setItem('slides', i, 'image', e.target.value)} className="field min-w-0" placeholder="Image URL" /><label className="touch shrink-0 cursor-pointer bg-mist py-3 text-sm">{uploading === `slides-${i}` ? 'Uploading…' : 'Upload'}<input type="file" accept="image/*" className="hidden" onChange={e => upload(e.target.files?.[0], 'slides', i)} /></label></div></div></div>)}</div></div>
    <div className="mt-5 rounded-2xl bg-white p-5 shadow-sm"><div><h2 className="font-semibold">Best sellers</h2><p className="mt-1 text-xs text-gray-500">These four products appear directly under the slideshow.</p></div><div className="mt-5 grid gap-5 sm:grid-cols-2">{bestSellers.slice(0, 4).map((item, i) => <div key={i} className="rounded-xl border border-gray-100 p-3"><img src={item.image} alt="" className="h-36 w-full rounded-lg object-cover" /><div className="mt-3 grid gap-2"><input value={item.name} onChange={e => setItem('bestSellers', i, 'name', e.target.value)} className="field" placeholder="Product name" /><input value={item.detail} onChange={e => setItem('bestSellers', i, 'detail', e.target.value)} className="field" placeholder="Details" /><div className="grid grid-cols-2 gap-2"><input type="number" value={item.price} onChange={e => setItem('bestSellers', i, 'price', Number(e.target.value))} className="field" placeholder="Offer price" /><input type="number" value={item.oldPrice || ''} onChange={e => setItem('bestSellers', i, 'oldPrice', e.target.value ? Number(e.target.value) : null)} className="field" placeholder="Old price" /></div><div className="flex gap-2"><input value={item.image} onChange={e => setItem('bestSellers', i, 'image', e.target.value)} className="field min-w-0" placeholder="Image URL" /><label className="touch shrink-0 cursor-pointer bg-mist py-3 text-sm">{uploading === `bestSellers-${i}` ? 'Uploading…' : 'Upload'}<input type="file" accept="image/*" className="hidden" onChange={e => upload(e.target.files?.[0], 'bestSellers', i)} /></label></div></div></div>)}</div></div>
