...
function ImagePicker({ onFile, label, loading = false, className = '' }) { const inputRef = useRef(null); return <><button type="button" disabled={loading} onClick={() => inputRef.current?.click()} className={`touch min-h-10 ${className}`}><Upload size={15} className="inline" /> {loading ? 'Uploading…' : label}</button><input ref={inputRef} type="file" accept="image/*" className="sr-only" onChange={event => { onFile(event.target.files?.[0]); event.target.value = '' }} /></> }
...
function ContentTransferPanel({ catalog, setCatalog, sourceServer }) {
  const servers = catalogServers(catalog)
  const sourceId = sourceServer?.id || null
  const products = sourceId ? (servers.find(server => server.id === sourceId)?.products || []) : (catalog.bestSellers || DEFAULT_CATALOG.bestSellers)
  const destinations = sourceId ? servers.filter(server => server.id !== sourceId) : servers
  const [productIndex, setProductIndex] = useState('0')
  const [targetId, setTargetId] = useState(destinations[0]?.id || '')
  useEffect(() => { setProductIndex('0'); setTargetId(destinations[0]?.id || '') }, [sourceId])
  const moveProduct = () => {
    const index = Number(productIndex)
    const product = products[index]
    const target = destinations.find(server => server.id === targetId)
    if (!product || !target) return
    if (target.products.length >= 12) return window.alert('That server already has 12 products.')
    const nextServers = servers.map(server => {
      if (server.id === targetId) return { ...server, products: [...server.products, product] }
      if (server.id === sourceId) return { ...server, products: server.products.filter((_, current) => current !== index) }
      return server
    })
    setCatalog({ ...catalog, bestSellers: sourceId ? (catalog.bestSellers || DEFAULT_CATALOG.bestSellers) : products.filter((_, current) => current !== index), servers: nextServers })
    setProductIndex('0')
  }
  if (!products.length || !destinations.length) return null
  return <div className="mt-5 rounded-xl border border-sand/30 bg-[#FCFAF7] p-4"><p className="text-sm font-semibold">Move product to a server</p><p className="mt-1 text-xs text-gray-600">Choose a product and its new server, then press Save all storefront changes.</p><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><select value={productIndex} onChange={event => setProductIndex(event.target.value)} className="field min-h-10 py-0 text-sm">{products.map((product, index) => <option key={`${product.name}-${index}`} value={index}>{product.name}</option>)}</select><select value={targetId} onChange={event => setTargetId(event.target.value)} className="field min-h-10 py-0 text-sm">{destinations.map(server => <option key={server.id} value={server.id}>{server.label}</option>)}</select><button type="button" onClick={moveProduct} className="touch min-h-10 bg-ink px-4 text-sm text-white">Move product</button></div></div>
}
...
function CatalogManager({ catalog, setCatalog, saveCatalog, saveStatus, server }) {
  ...
  const uploadProductImage = async (file, index, asMain = false) => {
    ...
    try {
      ...
      if (error) throw error
      const { data } = supabase.storage.from('store-assets').getPublicUrl(path)
      updateProduct(index, item => { const oldImages = productGallery(item).filter(image => !image.includes('placehold.co')); const images = asMain ? [data.publicUrl, ...oldImages] : [...oldImages, data.publicUrl]; return { ...item, image: images[0], images } })
    } catch (error) { window.alert(error.message || 'Image upload failed. Check your Supabase SQL setup.') }
    finally { setUploading('') }
  }
