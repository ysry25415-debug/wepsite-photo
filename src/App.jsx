...
function ContentTransferPanel({ catalog, setCatalog }) {
  const products = catalog.bestSellers || DEFAULT_CATALOG.bestSellers
  const servers = catalogServers(catalog)
  const [productIndex, setProductIndex] = useState(products.length ? '0' : '')
  ...
  const moveProduct = () => {
    ...
    if (!Number.isInteger(index) || !products[index] || !target) return
    if (target.products.length >= 12) return window.alert('That server already has the maximum of 12 products.')
    setCatalog({ ...catalog, bestSellers: products.filter((_, currentIndex) => currentIndex !== index), servers: servers.map(server => server.id === target.id ? { ...server, products: [...server.products, products[index]] } : server) })
    setProductIndex(products.length > 1 ? '0' : '')
  }
  if (!products.length) return null
  return <section className="mb-5 rounded-2xl border border-sand/30 bg-[#FCFAF7] p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[.2em] text-sand">Quick action</p><h2 className="mt-1 text-xl font-semibold">Move product to a server</h2><p className="mt-1 text-sm text-gray-600">Move one product from the main collection to Cars, Football, Singers, Games, or Art.</p><div className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><select value={productIndex} onChange={event => setProductIndex(event.target.value)} className="field min-h-11 py-0 text-sm">{products.map((product, index) => <option key={`${product.name}-${index}`} value={index}>{product.name}</option>)}</select><select value={targetServerId} onChange={event => setTargetServerId(event.target.value)} className="field min-h-11 py-0 text-sm">{servers.map(server => <option key={server.id} value={server.id}>{server.label}</option>)}</select><button type="button" onClick={moveProduct} className="touch min-h-11 bg-ink px-4 text-sm text-white">Move product</button></div></section>
}
...
function CatalogManager({ catalog, setCatalog, saveCatalog, saveStatus, server }) {
  ...
  return <><ContentTransferPanel catalog={catalog} setCatalog={setCatalog} /><form onSubmit={saveCatalog} className="max-w-4xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-sand">Storefront controls</p><h1 className="mt-1 text-3xl font-semibold">Your offers, products & prices</h1><p className="mt-2 text-sm text-gray-600">Upload the four home offers and manage every visible product price here. Press Save when you finish.</p>
